...
class ps3_P1A(unittest.TestCase):
    ...
    def test_clean_tile_at_position_PosToZero(self):
        ...
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(
                test.Position(x + random.random(), y + random.random()), dirt_amount
            )
    ...
    def test_clean_tile_at_position_PosToPos(self):
        ...
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(
                test.Position(x + random.random(), y + random.random()), dirt_amount - 1
            )
            # using random.random in case there is any issue with specific parts of a tile
        for x, y in xyrange(width, height):
            self.assertFalse(
    ...
    def test_get_random_position(self):
        ...
        for i in range(50000):
            ...
            self.assertTrue(
                ...
                "get_random_position returned {} which is not in [0, {}), [0, {})".format(
                    pos, width, height
                ),
            )
            x0, y0 = int(x), int(y)
        ...
        for m in range(7000):
            ...
            for r in robots:
                ...
                self.assertTrue(
                    ...
                    "which is not in [0, 5), [0, 7)" % ((p.get_x(), p.get_y()),),
                )
                self.assertTrue(
                    0 <= d < 360,
                    "Robot direction was set to %r, " "which is not in [0, 360)" % (d,),
                )
...
class SimulationTester(unittest.TestCase):
    def run_simulation(self, bounds, parameters):
        ...
        if thr.isAlive():
            self.fail(
                "Simulation took too long (more than %d seconds)" % SIMULATION_TIME_LIMIT
            )
        elif thr.getExceptionInfo():
            self.fail(
                "Exception occurred in simulation thread:\n\n%s"
...
class ps3_P5_Standard(SimulationTester):
    ...
    def testSimulation6(self):
        ...
        self.run_simulation(
            ((155, 180),), (3, 0.5, 1, 10, 10, 1, 0.9, 100, ps3.StandardRobot)
        )
    ...
    def testSimulation7(self):
        ...
        self.run_simulation(
            ((206, 266), (180, 240)), (1, 1.0, 3, 5, 5, 5, 1.0, 100, ps3.StandardRobot)
        )

    def testSimulation8(self):
        "Test cleaning 100% of a 5x5 room (Standard Robot, 6 dirt/tile, capacity = 3)"
        self.run_simulation(
            ((206, 266), (180, 240)), (1, 1.0, 3, 5, 5, 6, 1.0, 100, ps3.StandardRobot)
        )
    ...
    def testSimulation11(self):
        ...
        self.run_simulation(
            ((48, 108), (45, 104)), (3, 0.5, 2, 5, 5, 6, 0.9, 100, ps3.StandardRobot)
        )
