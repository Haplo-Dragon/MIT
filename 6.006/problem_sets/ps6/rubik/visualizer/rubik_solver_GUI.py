...
class Application:
    ...
    def update_cell(self, btn):
        ...
        self.fill_cell_properties(
            btn, self.faces_pos[face][0] + index % 2, self.faces_pos[face][1] + index / 2
        )

    # reset rubik to the initial state
    def reset_rubik(self):
        for f in list(self.faces.keys()):
            for i in range(4):
    ...
    def create_color_picker(self, f):
        ...
        for c in list(self.colors.keys()):
            button = Button(
                f, width=self.btnColor_w_defaut, height=self.btnColor_h_defaut
            )
    ...
    def display_arrow(self, turn_type):
        if turn_type == "U":
            self.set_all_arrows(
                [self.imgBlank, self.imgBlank, self.imgArrowLeft, self.imgArrowLeft]
                + [self.imgBlank] * 4
            )
        elif turn_type == "Ui":
            self.set_all_arrows(
                [self.imgBlank, self.imgBlank, self.imgArrowRight, self.imgArrowRight]
                + [self.imgBlank] * 4
            )
        elif turn_type == "L":
            self.set_all_arrows(
                [self.imgArrowDown, self.imgArrowDown] + [self.imgBlank] * 6
            )
        elif turn_type == "Li":
            ...
            self.set_all_arrows(
                [self.imgBlank] * 4
                + [
                    self.imgArrowTurn270,
                    self.imgArrowTurn0,
                    self.imgArrowTurn90,
                    self.imgArrowTurn180,
                ]
            )
        elif turn_type == "Fi":
            self.set_all_arrows(
                ...
                + [
                    self.imgArrowTurn0i,
                    self.imgArrowTurn90i,
                    self.imgArrowTurn180i,
                    self.imgArrowTurn270i,
                ]
            )
        else:
    ...
    def change_cell_color(self, btn, new_color):
        if len(self.steps) == 0:
            ...
            if face == "D" and index == 3:
                self.lblInfo[
                    "text"
                ] = "By convention, the logo yellow square \nshould be here before solving."
                return
            else:
                self.lblInfo["text"] = ""
    ...
    def btnArrow_clicked(self):
        if len(self.steps) > 0:
            self.lblInfo[
                "text"
            ] = "These arrows indicate the twisting \ndirection of the next move."
    ...
    def show_demo(self):
        if len(self.steps) == 0:
            ...
            self.lblInfo[
                "text"
            ] = "Here is a shuffled cube, please click \non Solve button to solve."
        else:
            self.lblInfo[
                "text"
            ] = "This will interrupt your solving process\nplease Reset first."
