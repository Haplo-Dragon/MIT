...
def distance(node1, node2):
    ...
    return (latitude_diff ** 2 + longitude_diff ** 2) ** 0.5
...
class NodeDistancePair(object):
    ...
    def __lt__(self, other):
        ...
        return self.distance < other.distance or (
            self.distance == other.distance and id(self.node) < id(other.node)
        )
...
class PathFinder(object):
    ...
    def shortest_path(self, weight):
        ...
        path, num_visited = self.dijkstra(
            weight, self.network.nodes, self.source, self.destination
        )

        time_used = round(time.clock() - start_time, 3)
        if path:
            if self.network.verify_path(path, self.source, self.destination):
                return PathResult(path, num_visited, weight, self.network, time_used)
        else:
            return None
...
"""
        ...
        kml.append(
            "".join(
                "%f,%f\n" % (node.longitude / 1000000.0, node.latitude / 1000000.0)
...
"""
    ...
    def to_lines(self):
        ...
        list = [
            "Path: %s, %s -> %s, %s"
            % (source.description, source.state, dest.description, dest.state)
        ]
