...
class TruthTable:
    ...
    def _build_table(self, output_list):
        ...
        if len(output_list) == 2:
            ...
            half = length // 2
            return [
                self._build_table(output_list[0:half]),
...
class Gate:
    ...
    def as_json(self):
        ...
        return {
            "id": self.name,
            "table": self.gate_type.truth_table.name,
...
class Transition:
    ...
    def apply(self):
        ...
        if self.gate.output == self.new_output:
            raise ValueError("Gate output should not transition to the same " "value")
...
class PriorityQueue:
    ...
    def append(self, key):
        ...
        Analogous to the MIN-HEAP-INSERT procedure.
        """
        if key is None:
            raise ValueError("Cannot insert None in the queue")
        # # Add a new node containing a temporary new Transition.
        # empty_transition = Transition()
        # self.queue.append(empty_transition)
        self.queue.append(key)
    ...
    def min(self):
        ...
        In a min-heap, this is always the root element (that is, the first element in
        the array.)
        """
        if len(self.queue) == 0:
            return None
    ...
    def pop(self):
        """Returns and removes the minimum element in the queue.

        Analogous to the EXTRACT_MIN procedure.
        """
        if len(self.queue) < 1:
            return None

        current_min = self.queue[0]

        # Put the end element at the top of the heap so it can trickle down to its
        # correct position.
        self.queue[0] = self.queue[-1]
        # Remove the node that used to be at the end of the heap by dropping it off the
        # end.
        self.queue = self.queue[:-1]
        # The new root may be in the wrong position, but its children are min-heaps,
        # so now we'll correct that by min-heapifying the root.
        self._min_heapify(0)
    ...
    def _decrease_key(self, index, key):
        ...
        # As long as the current node's parent is larger, the node needs to trickle
        # up the heap.
        while index > 0 and self.queue[self._parent(index)] > self.queue[index]:
            # Exchange the node with its parent.
            self.queue[index], self.queue[self._parent(index)] = \
                self.queue[self._parent(index)], self.queue[index]
            # Move up the tree one level.
            index = self._parent(index)

    def _min_heapify(self, index):
        # Get left and right children of the given node.
        left = self._left_child(index)
        right = self._right_child(index)

        # Determine the smallest element between the given node and its left and right
        # children.
        if (left < len(self.queue)) and (self.queue[left] < self.queue[index]):
            smallest = left
        else:
            smallest = index
        ...
        # If the given node IS the smallest element, then the current subtree is already
        # a min-heap, and we're done.
        if smallest != index:
            # Otherwise, one of the two children is the smallest element, so we'll swap
            # the given node and the smallest node with each other.
            self.queue[index], self.queue[smallest] = \
                self.queue[smallest], self.queue[index]
            # Then we'll make sure the new subtree is still a min-heap.
...
class Simulation:
    ...
    def step(self):
        ...
        while len(self.queue) > 0 and self.queue.min().time == step_time:
            ...
            if transition.gate.probed:
                self.probes.append(
                    [transition.time, transition.gate.name, transition.new_output]
                )
    ...
    def run(self):
        ...
        for in_transition in sorted(self.in_transitions):
            self.queue.append(
                Transition(in_transition[3], in_transition[2], in_transition[0])
            )
    ...
    def from_file(file):
        ...
        while True:
            ...
            if command[0] == "table":
                ...
                if len(command) != 4:
                    raise ValueError(
                        "Invalid number of arguments for gate type" " command"
                    )
                ...
                if len(command) != 2:
                    raise ValueError(
                        "Invalid number of arguments for gate " "probe command"
                    )
                circuit.add_probe(command[1])
            elif command[0] == "flip":
                if len(command) != 4:
                    raise ValueError("Invalid number of arguments for flip " "command")
                simulation.add_transition(command[1], int(command[2]), int(command[3]))
            elif command[0] == "done":
                break
        return simulation
    ...
    def outputs_to_line_list(self):
        return [
            " ".join([str(probe[0]), probe[1], str(probe[2])]) for probe in self.probes
        ]
