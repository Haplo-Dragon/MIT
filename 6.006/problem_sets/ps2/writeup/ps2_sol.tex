...
Because the cost of rasterization dominates the cost of compiling the line segments, at LOD 0, the CPU is doing $l^2$ work (that is, work proportional to the area of the triangle). At each subsequent level, each triangle is split into 4 triangles with sides $\frac{1}{3}$ as long.

So, at any given LOD $n$, the CPU is rasterizing $4^n$ triangles, each with area equal to $(\frac{l}{3^n})^2$. The total work at LOD $n$ is thus $4^n(\frac{l}{3}^n)^2$. A tiny bit of algebra later and the total work is $\frac{4^nl^2}{3^{n+2}}$.

Since we're assuming an initial side length of 1, the $l$ term will disappear, leaving us with $\frac{4^n}{3^{n+2}}$. Thanks to asymptotics, we can disregard the constant in the exponent, resulting in $\Theta(\frac{4^n}{3^n})$, or $\Theta(\frac{4}{3}^n)$
%%% PROBLEM 1(u) ANSWER END %%%
}
\newcommand{\answerIIa}{ \answer 
%%% PROBLEM 2(a) ANSWER START %%%
\texttt{PriorityQueue.\_find\_min()}
%%% PROBLEM 2(a) ANSWER END %%%
}
\newcommand{\answerIIb}{ \answer
%%% PROBLEM 2(b) ANSWER START %%%
During the profile test, \texttt{PriorityQueue.\_find\_min()} was called 259,964 times.

In the code itself, \texttt{PriorityQueue.\_find\_min()} is an internal class method, called when the public method \texttt{PriorityQueue.min()} is called (unless the queue size is 0). It is also called through the public method \texttt{PriorityQueue.pop()}. 

This means that, within the code, calls to \texttt{min()} and \texttt{pop()} are essentially calls to \texttt{\_find\_min()}. In the code, \texttt{\_find\_min()} is called in a total of 5 lines of code, including the times it is called through \texttt{min()} and \texttt{pop()}. It is called 3 times for each time step executed by \texttt{Simulation.step()}.
...
\handout{Problem Set \theproblemsetnum --- \textcolor{red}{Final Grade 83}} {\releasedate}
...
\problem \textcolor{red}{\points{23}} \textbf{Fractal Rendering}
...
\begin{problemparts}
  \problempart \textcolor{red}{\points{1}} What is the depth of the recursion tree for rendering a
  snowflake of LoD $n$?
    \begin{enumerate}
      \item $\log n$
      \item $n$
...
\answerIa
  ...
  \problempart \textcolor{red}{\points{2}} How many nodes are there in the recursion tree at level
  $i$, for $1 \le i \le n$?
    \begin{enumerate}
     \item $3 ^ i$
      \item $4 ^ i$
...
\answerIb
  ...
  \problempart \textcolor{red}{\points{1}} What is the asymptotic rendering time (triangle count) for a
...
\answerIc 
  ...
  \problempart \textcolor{red}{\points{1}} What is the asymptotic rendering time (triangle count) at
...
\answerId
  ...
  \problempart \textcolor{red}{\points{2}} What is the total asymptotic cost for the CPU, when rendering
...
\answerIf
  ...
  \problempart \textcolor{red}{\points{1}} How many nodes are there in the recursion tree at level
  $i$, for $1 \le i \le n$?
    \begin{enumerate}
      \item $3 ^ i$
      \item $4 ^ i$
...
\answerIg
  ...
  \problempart \textcolor{red}{\points{0}} What is the asymptotic rendering time (line segment count)
...
\answerIh
  ...
  \problempart \textcolor{red}{\points{1}} What is the asymptotic rendering time (line segment count)
...
\answerIi
  ...
  \problempart \textcolor{red}{\points{0}} What is the asymptotic rendering time (line segment count) at
  each level $i$ of the recursion tree, for $0 \le i < n$?
    \begin{enumerate}
      \item 0
      \item $\Theta(\frac{4}{9} ^ i)$
...
\answerIj
  ...
  \problempart \textcolor{red}{\points{1}} What is the asymptotic rendering time (line segment count) at
...
\answerIk
  ...
  \problempart \textcolor{red}{\points{1}} What is the total asymptotic cost for the CPU, when rendering
...
\answerIn
  ...
  \problempart \textcolor{red}{\points{0}} What is the asymptotic rendering time (line segment length)
...
\answerIo
  ...
  \problempart \textcolor{red}{\points{1}} What is the asymptotic rendering time (line segment length)
  for a node in the last level $n$ of the recursion tree?
    \begin{enumerate}
      \item $0$
      \item $\Theta(1)$
...
\answerIr
  ...
  \problempart \textcolor{red}{\points{0}} What is the total asymptotic cost for the CPU, when rendering
...
\begin{problemparts}
  \problempart \textcolor{red}{\points{0}} What is the total asymptotic cost of rendering a snowflake
...
\answerIt
  ...
  \problempart \textcolor{red}{\points{8}} Write a succinct proof for your answer using the  
...
\problem \textcolor{red}{\points{60}} \textbf{Digital Circuit Simulation}
...
\problempart \textcolor{red}{\points{8}} Run the code under the python profiler with the command
...
\problempart \textcolor{red}{\points{6}} How many times is the method called?
\answerIIb

\problempart \textcolor{red}{\points{8}} The class containing the troublesome method is
...
\problempart \textcolor{red}{\points{8}} If the data structure were implemented using the most
...
\problempart \textcolor{red}{\points{30}} Rewrite the data structure class using the most
