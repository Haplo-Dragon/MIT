...
\li $\id{low} \gets \proc{Zero}(N)$
\li $\id{high} \gets \proc{Div}(\proc{Copy}(A), K, N)$ \Comment{$\sqrt[K]{A}$ can't be larger than $A/K$.}
\li $\id{epsilon} \gets \proc{Word}(0.001)$
\zi
\li $\id{guess} \gets \proc{Div}(\proc{Add}(\id{low}, \id{high}, N), 2, N)$ \Comment{guess = (low + high) / 2}
\li $\id{guess^k} \gets \proc{Powmod}(\id{guess}, K, N)$
\zi
\li \Comment{While the absolute value of $guess^k - A$ is greater than epsilon...}
\li \While $\proc{Cmp}(|\proc{Sub}(\id{guess^k}, A)|, epsilon, N) \isequal \const{Greater}$
\li     \Do
            \If $\proc{Cmp}(\id{guess^k}, A, N) \isequal \const{Greater}$
\li             \Do
                    $\id{high} \gets \id{guess}$
                \End
\li         \If $\proc{Cmp}(\id{guess^k}, A, N) \isequal \const{Smaller}$
\li             \Do
                    $\id{low} \gets \id{guess}$
                \End
\li         \Else \Comment{Guess was equal (not strictly necessary because an equal guess would trip the}
\zi             \Comment{while condition above, but here for completeness).}
\li             \Do
                    break
                \End
\li         $\id{guess} \gets \proc{Div}(\proc{Add}(\id{low}, \id{high}, N), 2, N)$ \Comment{Make a new guess.}
\li         $\id{guess^k} \gets \proc{Powmod}(\id{guess}, K, N)$
        \End
\zi
\li \Return $\id{guess}$
...
\handout{Problem Set \theproblemsetnum \textcolor{red}{Final Grade 90}}{\releasedate}
...
\problem \points{18} \textbf{RSA Public-Key Encryption} \textcolor{red}{14/18 points}
...
\problem \points{42} \textbf{Image Decryption} \textcolor{red}{38/42 points}
