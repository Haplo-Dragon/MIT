...
class TracedResultSet(ResultSet):
    ...
    def add_crossing(self, wire1, wire2):
        self.trace.append({"type": "crossing", "id1": wire1.name, "id2": wire2.name})
...
class CrossVerifier(object):
    ...
    wire_crossings method) or count the crossings (count_crossings)."""
        ...
        self.index = rangeindex.RangeIndex()
    ...
    def _events_from_layer(self, layer):
        ...
        The number 0, 1, or 2 in the event list allows us to order events that occur
        at the same x-coordinate. ADD events occur first, followed by QUERY events,
        then REMOVE events. This is hacky and dumb, but I haven't come up with anything
        better yet.
        """
        for wire in layer.wires.values():
            if wire.is_horizontal():
                # When the sweep line hits the left edge of a horizontal wire,
                # the wire is added to the index.
                self.events.append([wire.x1, 0, wire.object_id, EVENT.ADD, wire])
                # When the sweep line hits the right edge of a horizontal wire,
                # the wire is removed from the index.
                self.events.append([wire.x2, 2, wire.object_id, EVENT.REMOVE, wire])
            else:
                # When the sweep line hits a vertical wire, a query is performed.
                self.events.append([wire.x1, 1, wire.object_id, EVENT.QUERY, wire])

    def _compute_crossings(self, count_only):
        if count_only:
            # This is the case when we're only interested in how many crossings
            # there are.
            result = 0
        else:
            # Otherwise, we want to know which wires are crossing.
            result = self.result_set

        for event in self.events:
            event_x, event_type, wire = event[0], event[3], event[4]

            # Draw the sweep line for the trace visualizer.
            self.trace_sweep_line(event_x)

            if event_type == EVENT.ADD:
                # Left edge of horizontal wire - add the wire to the range index.
                self.index.add(KeyWirePair(wire.y1, wire))
                # size = self.index.root.size if self.index.root else 0
                # print("Added node, index now has {} nodes.".format(size))
            elif event_type == EVENT.REMOVE:
                # Right edge of horizontal wire - remove the wire from the range index.                
                self.index.remove(KeyWirePair(wire.y1, wire))
                # size = self.index.root.size if self.index.root else 0
                # print("Removed node, index now has {} nodes.".format(size))
            elif event_type == EVENT.QUERY:
                # Vertical wire - check for any horizontal wires that cross it.

                if count_only:
                    # print("Index has {} nodes.".format(self.index.root.size))
                    result += self.index.count(
                        KeyWirePairL(wire.y1),
                        KeyWirePairH(wire.y2))
                else:
                    # Check for wire crossings.
                    # This KeyWirePairL and H stuff seems like an ugly hack. Does it need
                    # to be fixed, too?
                    # Essentially, we're listing all the wires in the index that fall
                    # along the length of the given vertical wire.
                    for kwp in self.index.list(
                            KeyWirePairL(wire.y1), KeyWirePairH(wire.y2)):
...
class TracedCrossVerifier(CrossVerifier):
    ...
    def __init__(self, layer):
        ...
        self.trace = []
        self.index = rangeindex.TracedRangeIndex(self.trace)
